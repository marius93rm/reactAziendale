      ...
      <Paper component="section" variant="outlined" sx={{ p: { xs: 2.5, md: 3 } }}>
        ...
        >
          ...
          <Stack spacing={1} sx={{ minWidth: 180 }}>
            <Typography>{describeWeatherCode(forecast.current.weatherCode)}</Typography>
            <Typography color="text.secondary">
...
function ForecastContent({
  ...
  if (state.status === 'idle') {
    return (
      <Alert icon={<CloudOutlinedIcon fontSize="inherit" />} severity="info">
        ...
        Completa i TODO del brief per caricare le condizioni della sede selezionata.
      </Alert>
    );
  }
  ...
  if (state.status === 'error') {
    return (
      <Alert
        severity="error"
        action={<Button color="inherit" onClick={retry}>Riprova</Button>}
      >
  ...
  if (state.status === 'empty') {
    return (
      <Alert
        severity="info"
        action={<Button color="inherit" onClick={refresh}>Aggiorna</Button>}
      >
